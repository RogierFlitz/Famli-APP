...
import { neededCategoryLabel, neededLocationLabel } from "@/lib/domain/labels";
...
import {
  ...
  unmarkNeededItemBoughtAction,
} from "@/lib/actions/life";
import type { FamilySnapshot, NeededItem } from "@/lib/domain/types";
import { cn } from "@/lib/utils";
import { NeededToggle } from "@/components/children/needed-toggle";
...
export function NeededList({
  ...
  const filtered = items.filter((item) => (giftsOnly ? item.category === "cadeau" : item.category !== "cadeau" || !compact));
  const openItems = filtered.filter((item) => item.status !== "gekocht" && item.status !== "niet_meer_nodig");
  const completedItems = showCompleted
    ? filtered.filter((item) => item.status === "gekocht")
    : [];
  ...
  return (
    <div className="space-y-3">
      ...
      {showCompleted && completedItems.length ? (
        <CompletedRegelenSection snapshot={snapshot} tasks={[]} neededItems={completedItems} />
      ) : null}
    </div>
  );
}
...
function NeededCard({
  ...
  const isBought = item.status === "gekocht";
  const canToggle = canEdit && !item.expenseId;

  if (isBought) {
    return (
      <article className="famli-card opacity-80">
        <div className="flex items-start gap-3">
          {canToggle ? (
            <NeededToggle itemId={item.id} checked title={item.title} />
          ) : null}
          <div className="min-w-0 flex-1">
            <p className="text-lg font-medium line-through">{item.title}</p>
            <p className="text-sm text-[color:var(--famli-muted)]">
              ✓ Gekocht
              {item.purchasedAt ? ` · ${formatCompletedAt(item.purchasedAt)}` : ""}
              {item.purchasedByMemberId ? ` · door ${parentName(snapshot, item.purchasedByMemberId)}` : ""}
            </p>
          </div>
          {canToggle ? (
            <ReopenButton compact onReopen={() => unmarkNeededItemBoughtAction(item.id)} />
          ) : null}
        </div>
        {canEdit && !item.expenseId && snapshot.currentMember.role !== "viewer" ? (
          <form
            className="mt-3"
            action={async (formData) => {
  ...
  return (
    <article className="famli-card space-y-2">
      <div className="flex items-start gap-3">
        {canEdit ? <NeededToggle itemId={item.id} checked={false} title={item.title} /> : null}
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {item.location && item.location !== "onbekend" ? (
            <p className="text-sm text-[color:var(--famli-muted)]">
              Locatie:{" "}
              {item.location === "custom" && item.locationCustom
                ? item.locationCustom
                : neededLocationLabel[item.location]}
            </p>
          ) : null}
          ...
          {canEdit ? (
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              <button
                type="button"
                className="famli-btn famli-btn-secondary h-10 px-4"
          ...
          {detailsOpen ? (
            ...
            >
              ...
              <button className="famli-btn famli-btn-primary w-full">Opslaan als gekocht</button>
            </form>
          ) : null}
        </div>
      </div>
